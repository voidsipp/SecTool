 ...
 *   GET  /api/cve?hours=N           -> CVE-exposure / exploited-vulnerability report (named CVEs ranked by unblocked pressure = patch worklist; model + Markdown)
...
export async function startWebServer(cfg: Config): Promise<WebServer> {
  ...
  const handler = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      // --- data-coverage / quality (dataset-integrity audit) ---
      if (method === "GET" && path === "/api/coverage") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 6;
        return send(res, 200, buildCoverage(hours, { limit, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/coverage.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 6;
        const now = Date.now();
        const { markdown } = buildCoverage(hours, { limit, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
      ...
      // --- AI analyst-insight digest (summary coverage / re-grading / actions) ---
      if (method === "GET" && path === "/api/insight") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 15;
        return send(res, 200, buildInsight(hours, { limit, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/insight.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 15;
        const now = Date.now();
        const { markdown } = buildInsight(hours, { limit, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
      ...
      // --- CVE-exposure / exploited-vulnerability (patch worklist) ---
      if (method === "GET" && path === "/api/cve") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        return send(res, 200, buildCve(hours, { limit, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/cve.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        const now = Date.now();
        const { markdown } = buildCve(hours, { limit, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
          "content-disposition": `attachment; filename="${cveFilename(now)}"`,
        });
        res.end(markdown);
        return;
      }
      ...
      if (method === "GET" && path === "/api/cloud") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 25;
        return send(res, 200, buildCloud(hours, { limit, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/cloud.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 25;
        const now = Date.now();
        const { markdown } = buildCloud(hours, { limit, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
      ...
      // --- attack-surface-by-service-class (remote-access/database/file-share/ICS-IoT roll-up + exposed crown-jewel worklist) ---
      if (method === "GET" && path === "/api/services") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        return send(res, 200, buildServices(hours, { limit, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/services.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        const now = Date.now();
        const { markdown } = buildServices(hours, { limit, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
          "content-disposition": `attachment; filename="${servicesFilename(now)}"`,
        });
        res.end(markdown);
        return;
      }
      ...
      // --- source dwell-time / engagement-session (sustained camp vs transient probe per source) ---
      if (method === "GET" && path === "/api/dwell") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        const minAlerts = Number(url.searchParams.get("minAlerts")) || undefined;
        const gapMinutes = Number(url.searchParams.get("gap")) || undefined;
        return send(res, 200, buildDwell(hours, { limit, minAlerts, gapMinutes, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/dwell.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        const minAlerts = Number(url.searchParams.get("minAlerts")) || undefined;
        const gapMinutes = Number(url.searchParams.get("gap")) || undefined;
        const now = Date.now();
        const { markdown } = buildDwell(hours, { limit, minAlerts, gapMinutes, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
      ...
      // --- suppression-rule audit / silence-effectiveness & risk ---
      if (method === "GET" && path === "/api/suppaudit") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || undefined;
        const graceHours = Number(url.searchParams.get("grace")) || undefined;
        return send(res, 200, buildSuppressionAudit(hours, { limit, graceHours, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/suppaudit.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || undefined;
        const graceHours = Number(url.searchParams.get("grace")) || undefined;
        const now = Date.now();
        const { markdown } = buildSuppressionAudit(hours, { limit, graceHours, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
      ...
      // --- temporal-convergence / coordinated-strike (botnet / DDoS / distributed-spray flash crowd) ---
      if (method === "GET" && path === "/api/convergence") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        const minSources = Number(url.searchParams.get("minSources")) || undefined;
        const windowSec = Number(url.searchParams.get("window")) || undefined;
        return send(res, 200, buildConvergence(hours, { limit, minSources, windowSec, nowMs: Date.now() }));
      }
      if (method === "GET" && path === "/api/convergence.md") {
        const hours = Number(url.searchParams.get("hours")) || cfg.web.defaultHours;
        const limit = Number(url.searchParams.get("limit")) || 20;
        const minSources = Number(url.searchParams.get("minSources")) || undefined;
        const windowSec = Number(url.searchParams.get("window")) || undefined;
        const now = Date.now();
        const { markdown } = buildConvergence(hours, { limit, minSources, windowSec, nowMs: now });
        res.writeHead(200, {
          "content-type": "text/markdown; charset=utf-8",
          "cache-control": "no-store",
          "content-disposition": `attachment; filename="${convergenceFilename(now)}"`,
        });
        res.end(markdown);
        return;
      }
