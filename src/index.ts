...
async function main(): Promise<void> {
  ...
  try {
    ...
    const protocolsIdx = argv.findIndex((a) => a === "--protocols" || a.startsWith("--protocols="));
    if (protocolsIdx !== -1) {
      const inline = argv[protocolsIdx]!.split("=")[1];
      const next = argv[protocolsIdx + 1];
      const raw = inline ?? (next && !next.startsWith("--") ? next : undefined);
      // Default to a week so the protocol mix reflects more than one shift.
      const hours = raw ? Number(raw) : 168;
      if (!Number.isFinite(hours) || hours <= 0) {
        log.error(`Invalid --protocols hours: "${raw}". Use e.g. --protocols 168`);
        process.exit(2);
      }
      // Optional `--limit N` to cap each per-protocol table.
      let limit = 20;
      const limitIdx = argv.findIndex((a) => a === "--limit" || a.startsWith("--limit="));
      if (limitIdx !== -1) {
        const li = argv[limitIdx]!.split("=")[1] ?? argv[limitIdx + 1];
    ...
    // Cloud / hosting-origin attribution — match public source IPs to cloud/VPS/CDN providers.
    const cloudIdx = argv.findIndex((a) => a === "--cloud" || a.startsWith("--cloud="));
    if (cloudIdx !== -1) {
      const inline = argv[cloudIdx]!.split("=")[1];
      const next = argv[cloudIdx + 1];
      const raw = inline ?? (next && !next.startsWith("--") ? next : undefined);
      // Default to a week so the provider mix reflects more than one shift.
      const hours = raw ? Number(raw) : 168;
      if (!Number.isFinite(hours) || hours <= 0) {
        log.error(`Invalid --cloud hours: "${raw}". Use e.g. --cloud 168`);
        process.exit(2);
      }
      // Optional `--limit N` to cap the offending-source table.
      let limit = 25;
    ...
    const silenceIdx = argv.findIndex((a) => a === "--silence" || a.startsWith("--silence="));
    if (silenceIdx !== -1) {
      const inline = argv[silenceIdx]!.split("=")[1];
      const next = argv[silenceIdx + 1];
      const raw = inline ?? (next && !next.startsWith("--") ? next : undefined);
      // Default to a week so there is a real established baseline to fall silent from.
      const hours = raw ? Number(raw) : 168;
      if (!Number.isFinite(hours) || hours <= 0) {
        log.error(`Invalid --silence hours: "${raw}". Use e.g. --silence 168`);
        process.exit(2);
      }
      // Optional `--limit N` to cap each per-dimension leaderboard.
      let limit = 15;
      ...
      // Optional `--quiet H` for the trailing window that must be alert-free (hours).
      let quietHours: number | undefined;
      const quietIdx = argv.findIndex((a) => a === "--quiet" || a.startsWith("--quiet="));
      if (quietIdx !== -1) {
        const v = argv[quietIdx]!.split("=")[1] ?? argv[quietIdx + 1];
        const n = v !== undefined ? Number(v) : NaN;
        if (Number.isFinite(n) && n > 0) quietHours = n;
      }
      // Optional `--min-count N` for how loud a fixture must have been to flag.
      let minCount: number | undefined;
      const minIdx = argv.findIndex((a) => a === "--min-count" || a.startsWith("--min-count="));
      if (minIdx !== -1) {
        const v = argv[minIdx]!.split("=")[1] ?? argv[minIdx + 1];
    ...
    if (diversityIdx !== -1) {
      const inline = argv[diversityIdx]!.split("=")[1];
      const next = argv[diversityIdx + 1];
      const raw = inline ?? (next && !next.startsWith("--") ? next : undefined);
      // Default to a week so a real population of entities spreads across the indices.
      const hours = raw ? Number(raw) : 168;
      if (!Number.isFinite(hours) || hours <= 0) {
        log.error(`Invalid --diversity hours: "${raw}". Use e.g. --diversity 168`);
        process.exit(2);
      }
      const cfg = loadConfig();
      setLogLevel(cfg.runtime.logLevel);
      // Offline, deterministic: print the Markdown diversity report to stdout.
      console.log(buildDiversity(hours, { nowMs: Date.now() }).markdown);
      return;
    }
    const originsIdx = argv.findIndex((a) => a === "--origins" || a.startsWith("--origins="));
    if (originsIdx !== -1) {
      const inline = argv[originsIdx]!.split("=")[1];
      const next = argv[originsIdx + 1];
      const raw = inline ?? (next && !next.startsWith("--") ? next : undefined);
      // Default to a week so a real population of sources spreads across regions.
      const hours = raw ? Number(raw) : 168;
      if (!Number.isFinite(hours) || hours <= 0) {
        log.error(`Invalid --origins hours: "${raw}". Use e.g. --origins 168`);
        process.exit(2);
      }
      // Optional `--limit N` to cap how many (busiest) offending-source rows print.
      let limit = 25;
      const limitIdx = argv.findIndex((a) => a === "--limit" || a.startsWith("--limit="));
      if (limitIdx !== -1) {
        const li = argv[limitIdx]!.split("=")[1] ?? argv[limitIdx + 1];
      ...
      const cfg = loadConfig();
      setLogLevel(cfg.runtime.logLevel);
      // Offline, deterministic: print the Markdown regional-origins report to stdout.
      console.log(buildOrigins(hours, { limit, nowMs: Date.now() }).markdown);
      return;
    }
    ...
    // Zeek (Bro) Intelligence Framework export (feed observed attackers into an NSM's intel framework).
    const zeekIdx = argv.findIndex((a) => a === "--zeek" || a.startsWith("--zeek="));
    if (zeekIdx !== -1) {
      const inline = argv[zeekIdx]!.split("=")[1];
      const next = argv[zeekIdx + 1];
      const raw = inline ?? (next && !next.startsWith("--") ? next : undefined);
      const hours = raw ? Number(raw) : 168;
      if (!Number.isFinite(hours) || hours <= 0) {
        log.error(`Invalid --zeek hours: "${raw}". Use e.g. --zeek 168`);
        process.exit(2);
      }
      // Optional `--format intel|script|json|md` — the Intel::ADDR `.dat` by default.
      const fmtIdx = argv.findIndex((a) => a === "--format" || a.startsWith("--format="));
      const fmtRaw = fmtIdx !== -1 ? (argv[fmtIdx]!.split("=")[1] ?? argv[fmtIdx + 1]) : undefined;
      const format = parseZeekFormat(fmtRaw);
      // Optional `--min-severity info|low|medium|high|critical` (default medium).
      const sevIdx = argv.findIndex((a) => a === "--min-severity" || a.startsWith("--min-severity="));
      const sevRaw = sevIdx !== -1 ? (argv[sevIdx]!.split("=")[1] ?? argv[sevIdx + 1]) : undefined;
      const minSeverity = parseSeverityFloor(sevRaw);
      ...
      // Optional `--source NAME` overrides the `meta.source` label (default SecTool).
      const srcIdx = argv.findIndex((a) => a === "--source" || a.startsWith("--source="));
      const source = srcIdx !== -1 ? (argv[srcIdx]!.split("=")[1] ?? argv[srcIdx + 1]) : undefined;
      const includeSafe = args.has("--include-safe");
      // `--notice` adds meta.do_notice=T so a hit also raises a Zeek Notice (off by default).
      const notice = args.has("--notice");
      const cfg = loadConfig();
      setLogLevel(cfg.runtime.logLevel);
      // Offline, deterministic: print the intel file / loader (or the JSON model / Markdown twin).
