...
export const REPORT_CATALOG: readonly CatalogEntry[] = [
  ...
  entry("Quality / AI / ops", "misp", "--misp", "/api/misp", 168,
    "MISP Event export", "Native MISP (Malware Information Sharing Platform) Event — the ecosystem-native sibling of stix. Each attacker IP becomes one ip-src Attribute (\"Network activity\") on a single Event, with deterministic UUIDv5 ids (idempotent re-publish), the to_ids flag set from SecTool's confidence (default ≥60, so MISP's publish-to-IDS workflow inherits the triage), per-attribute sectool: taxonomy tags (severity, confidence band, category) and an event-level TLP tag. Event metadata maps to MISP's own vocabulary: threat_level_id from worst severity, analysis=Initial, distribution=org-only and published=false (set the sharing scope deliberately after review). Ingest via PyMISP add_event, REST /events/add, or a static MISP feed. Options: --tlp, --to-ids N, --min-severity, --limit, --include-safe, --format md. Reuses the iocs scoring engine; safelisted IPs excluded by default."),
  ...
  entry("Quality / AI / ops", "csv", "--csv", "/api/csv", 168,
    "Tabular CSV / TSV alert export", "The analyst's \"give me the spreadsheet\" export — one flat row per alert, ready for Excel / Google Sheets / pandas / SQLite / awk, no SIEM required. Distinct from the dashboard's query-driven /api/search.csv: a first-class export-family member (CLI + npm + /api/csv[.csv|.tsv|.json|.md]) that enriches each row with the same re-parsed network fields cef/ecs recover — source/destination ports, transport + application protocol, the gid:sid rule identity, traffic direction and coarse disposition — plus live enforcement flags (blocked/watched/safelisted), id, severity, classification, notified/has-summary and the raw line. Output is RFC 4180 (CRLF records, quoted fields) with spreadsheet formula-injection hardening; --format csv (default)|tsv|json|md, --bom for an Excel-friendly UTF-8 BOM, --limit N. The flat-table sibling of the cef/ecs SIEM event exports.", false),
  ...
  entry("Quality / AI / ops", "zeek", "--zeek", "/api/zeek", 168,
    "Zeek (Bro) Intelligence Framework export", "The network-security-monitor sibling of snort the export family was missing: turns observed attacker IPs into a ready-to-load Zeek Intelligence Framework file so a Zeek / Corelight NSM raises an intel.log hit (and optionally a Notice) the instant a known-bad address reappears in any protocol it parses — a connection, DNS query, HTTP request, TLS handshake or file transfer. Reuses the iocs confidence/severity/safelist model. Emits a strictly-valid tab-separated Intel::ADDR `.dat` (no comment lines — the ASCII input reader rejects them; provenance lives in meta.desc + an AbuseIPDB meta.url pivot), --format script for a portable @load-able loader that wires the file in via @DIR, json or md (review twin); --notice adds meta.do_notice=T (raise a Zeek Notice on a hit, off by default), --source NAME relabels meta.source, --min-severity, --limit N. Zeek MONITORS, it does not block — pair with fwrules/snort to enforce; distinct from snort/sigma (signature/SIEM detection rules), stix/iocs (intel) and pcap (raw-packet capture).", false),
  ...
  entry("Quality / AI / ops", "dns", "--dns", "/api/dns", 168,
    "DNS sinkhole / blocklist export", "The DNS-layer enforcement export the family was missing — every other export (iocs/fwrules/snort/pcap) stops at the IP. Re-mines the malicious *domains* riding inside alert payloads (HTTP Host / TLS SNI / DNS query / URL host, reusing the conservative artifacts extractor) and renders them as a ready-to-load DNS blocklist: --format hosts (default 0.0.0.0 lines), dnsmasq (address=/d/ip, wildcards subdomains), unbound (local-zone always_nxdomain), rpz (a complete BIND Response Policy Zone), pihole (bare adlist), json or md (review twin). A built-in benign-infrastructure skip-list (CDN/OS-update/cloud/cert/NTP/reverse-DNS) is on by default so legit names aren't black-holed (--include-benign overrides); --sinkhole IP, --min N alert floor, --min-severity, --limit N. Confidence-ranked, observed-not-judged — review before loading. The domain-axis sibling of fwrules (IP firewall codegen) and the enforcement step after artifacts (which mines but doesn't block).", false),
  entry("Quality / AI / ops", "ics", "--ics", "/api/ics", 168,
    "iCalendar (.ics) security calendar", "The one downstream the other exports skip — the calendar. Renders the recent alert history as a valid RFC 5545 iCalendar: one all-day VEVENT per UTC day carrying that day's headline numbers (volume, serious count, unique sources/targets/signatures, busiest source, top signature, dominant category, new attackers) plus a day-over-day delta and notable-day flags (busiest day, day-over-day spike, critical-severity day, new-attacker influx). Drop the .ics into Outlook / Google / Apple Calendar / Thunderbird, or — far better — subscribe to webcal://<host>/api/ics.ics so the security calendar refreshes itself hourly (X-PUBLISHED-TTL/REFRESH-INTERVAL). Days at/above the alarm floor raise a reminder VALARM; every event is free/TRANSPARENT so it never blocks your availability. --format json|md, --include-quiet emits zero-alert days too, --no-alarms suppresses reminders, --limit N. The calendar-app companion to timeline (Markdown ledger) and feed (RSS/Atom item stream); distinct consumer, distinct shape — time-anchored day blocks with alarms, not a newest-first item feed.", false),
